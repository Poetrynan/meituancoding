...
export const DisputeModal: React.FC<DisputeModalProps> = ({ contract, isOpen, onClose }) => {
  ...
  return ReactDOM.createPortal(
    ...
    >
      <div
        className="bg-white rounded-3xl max-w-xl w-full border-2 border-craft-terracotta/40 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in"
        onClick={(e) => e.stopPropagation()}
      >
        {/* 顶部标题 */}
        <div className="p-6 pb-4 border-b border-craft-border flex items-center justify-between bg-craft-terracotta-light flex-shrink-0">
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
          ...
          <div className="p-4 border-t border-craft-border flex items-center justify-end gap-3 bg-craft-paper flex-shrink-0">
            <button
              type="button"
              onClick={onClose}
            ...
            <button
              type="submit"
              className="px-5 py-2 rounded-xl bg-craft-terracotta text-white text-xs font-bold hover:bg-craft-terracotta-dark shadow-sm flex items-center gap-1.5 cursor-pointer"
            >
